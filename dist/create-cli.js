...
export function isRemoteMcpCommand(name) {
    if (MCP_REMOTE_EXCLUDED_COMMANDS.has(name))
        return false;
...
function isLocalMcpCommand(name) {
    if (MCP_EXCLUDED_COMMANDS.has(name))
        return false;
    if (name.startsWith('completions'))
        return false;
    return true;
}
export function createCli({ version } = {}) {
    ...
    const cli = goke('akarso')
        .option('--api-key [key]', z
        .string()
        .describe('API key (overrides AKARSO_API_KEY env and ~/.akarso/config.json)'))
        .option('--api-url [url]', z
        .string()
        .describe('Server URL (overrides AKARSO_API_URL env, defaults to akarso.co)'))
        .option('--profile [id]', z
        .string()
        .describe('Profile (workspace) ID to operate on, overriding the API key\'s pinned profile. Use `profiles list` to see available IDs.'))
