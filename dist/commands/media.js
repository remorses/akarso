...
import { createGroup } from "../globals.js";
import { createClient, resolveApiKey, resolveBaseUrl } from "../client.js";
...
export function classifyMediaInput(input) {
    ...
    if (!contentType && !isUrl) {
        return new Error(`Unsupported file type "${ext || input}". Supported extensions: ${SUPPORTED_EXTENSIONS}`);
    }
    return { kind: isUrl ? 'url' : 'path', contentType, filename };
}
/** Error message for local paths on the hosted multi-tenant MCP server. */
function remotePathError(input) {
...
export async function uploadMedia(opts) {
    ...
    if (opts.env.AKARSO_REMOTE_MCP)
        throw remotePathError(opts.input);
    if (!info.contentType) {
        throw new Error(`Unsupported file type for "${opts.input}". Supported extensions: ${SUPPORTED_EXTENSIONS}`);
    }
...
media
    .command('media upload <fileOrUrl>', dedent `
    ...
    output({ id: result.id, url: result.url, type: result.type }, { json: options.json, console });
});
