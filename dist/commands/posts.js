...
import { resolveMediaToUploadId } from "./media.js";
const POST_STATUSES = [
...
export function buildPostBody(opts) {
    ...
    const postDate = opts.scheduledAt
        ...
        : now.toISOString();
    const status = opts.publishNow || opts.scheduledAt ? 'SCHEDULED' : 'DRAFT';
    ...
    return {
        ...
        status,
        socialAccountTypes,
        data: Object.fromEntries(socialAccountTypes.map((type) => [
            type,
...
posts
    ...
    .option('--platforms <list>', z.string().describe(`Comma-separated platforms to publish to (${platforms.schema.options.join(', ')})`))
    .option('--publish-now', 'Publish immediately')
    .option('--scheduled-at [iso]', z.string().describe('Schedule time: ISO date, 30m, 2h, 3d, or 1w'))
    .option('--title [title]', z.string().describe('Internal post title, shown in the dashboard (defaults to the text)'))
    .option('--media [items]', z
    .string()
    .describe('Comma-separated media items: local file paths or `https` URLs. Both are uploaded and attached to every platform (local paths are not available on the hosted MCP server, use URLs there).'))
...
posts
    ...
    `)
    .option('--status [status]', z.enum(POST_STATUSES).describe('Filter by status'))
    .option('--platforms [list]', z.string().describe('Comma-separated platforms to filter by'))
    .option('--search [q]', z.string().describe('Search post titles and content'))
    .option('--limit [n]', z.number().default(10).describe('Maximum number of posts to return'))
    .action(async (options, { fs, console, process }) => {
    const client = await createClient({
        apiKey: options.apiKey,
        fs,
...
posts
    .command('posts get <postId>', dedent `
...
posts
    .command('posts delete <postId>', dedent `
...
posts
    .command('posts retry <postId>', dedent `
